...
export function Landing({ player }: { player: ReturnType<typeof useAudioPlayer> }) {
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      {/* Hero */}
      <section className="relative pt-32 pb-16 text-center overflow-hidden bg-accent-dark">
        <div className="absolute top-[-200px] left-1/2 -translate-x-1/2 w-[700px] h-[700px] bg-accent/20 rounded-full blur-[120px] pointer-events-none" />
        <div className="absolute top-[100px] right-[-100px] w-[300px] h-[300px] bg-purple-500/10 rounded-full blur-[80px] pointer-events-none" />
        <div className="relative max-w-3xl mx-auto px-5">
          <div className="inline-flex items-center gap-1.5 px-4 py-1.5 bg-white/10 border border-white/15 rounded-full text-purple-300 text-xs font-medium mb-8">
            ✝ Revelation of Christ Ministries
          </div>
          <h1 className="text-5xl sm:text-6xl font-black leading-[1.05] tracking-tight mb-5 text-white">
            Grow in the<br />
            <span className="bg-gradient-to-r from-purple-400 via-violet-300 to-amber-400 bg-clip-text text-transparent">Knowledge of Christ</span>
          </h1>
          ...
          <div className="flex gap-3 justify-center flex-wrap mb-10">
            <Link to="/bible-study" className="inline-flex items-center gap-2 bg-accent text-white px-6 py-3 rounded-full font-semibold shadow-lg shadow-accent/30 hover:-translate-y-0.5 active:scale-[0.98] transition no-underline">
              <span className="text-xs">▶</span> Listen to Bible Study
            </Link>
            <Link to="/school-of-prayer" className="inline-flex items-center bg-white/10 border border-white/15 text-white px-6 py-3 rounded-full font-semibold hover:bg-white/15 active:scale-[0.98] transition no-underline">
              🙏 School of Prayer
            </Link>
          </div>
          ...
          {latestEp && (
            <div className="max-w-lg mx-auto bg-white/10 backdrop-blur-sm border border-white/10 rounded-2xl p-4 shadow-xl text-left">
              <p className="text-[10px] font-bold uppercase tracking-[2px] text-purple-300 mb-2">Latest Episode</p>
              <div className="flex items-center gap-3">
                <button onClick={() => player.play(latestEp)}
                  className="w-12 h-12 rounded-full bg-accent text-white flex items-center justify-center flex-shrink-0 shadow-lg shadow-accent/30 hover:scale-105 active:scale-95 transition">
                  {player.current?.audioUrl === latestEp.audioUrl && player.playing ? '⏸' : '▶'}
                </button>
                <div className="flex-1 min-w-0">
                  <h3 className="text-sm font-bold truncate text-white">{latestEp.title}</h3>
                  <p className="text-xs text-gray-400">{latestEp.date} {latestEp.duration && `· ${latestEp.duration}`}</p>
                </div>
              </div>
            </div>
          )}
      ...
      <section className="py-6 bg-surface border-b border-gray-100">
      ...
      <section className="py-16 bg-accent-dark">
        <div className="max-w-3xl mx-auto px-5">
          <div className="flex flex-col sm:flex-row items-center gap-8">
            <div className="w-40 h-40 sm:w-48 sm:h-48 bg-gradient-to-br from-accent via-purple-600 to-purple-800 rounded-3xl flex items-center justify-center flex-shrink-0 shadow-2xl shadow-accent/30">
            ...
            <div className="text-center sm:text-left">
              ...
              <div className="bg-white/5 border-l-[3px] border-accent rounded-r-xl p-3 text-left inline-block">
                ...
                <span className="text-purple-400 text-xs font-semibold">— Acts 26:18</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-20 bg-accent-dark text-center">
        <div className="max-w-3xl mx-auto px-5">
          <span className="text-xs font-semibold uppercase tracking-[2.5px] text-purple-400 block mb-1">Stay Connected</span>
          <h2 className="text-3xl font-extrabold mb-2 text-white">Never Miss a Teaching</h2>
          <p className="text-gray-400 mb-8 text-lg">Subscribe on your favourite podcast platform.</p>
          <div className="flex gap-3 justify-center flex-wrap mb-10">
            <a href="https://open.spotify.com/show/4VnpGdSCkRyJr0tgHELfwP" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-5 py-3.5 bg-white/10 border border-[#1DB954]/30 rounded-2xl no-underline text-white font-semibold hover:border-[#1DB954]/60 hover:-translate-y-0.5 active:scale-[0.98] transition shadow-sm">
            ...
            <a href="https://podcasts.apple.com/podcast/id1772578109" target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-2 px-5 py-3.5 bg-white/10 border border-purple-400/30 rounded-2xl no-underline text-white font-semibold hover:border-purple-400/60 hover:-translate-y-0.5 active:scale-[0.98] transition shadow-sm">
          ...
          {/* Listen on site */}
          <p className="text-sm text-gray-500 mb-4">Or listen right here:</p>
          <div className="flex gap-3 justify-center flex-wrap">
            <Link to="/bible-study" className="flex items-center gap-2 px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl no-underline text-gray-300 font-semibold hover:border-white/20 hover:-translate-y-0.5 active:scale-[0.98] transition shadow-sm">📖 Bible Study</Link>
            <Link to="/school-of-prayer" className="flex items-center gap-2 px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl no-underline text-gray-300 font-semibold hover:border-white/20 hover:-translate-y-0.5 active:scale-[0.98] transition shadow-sm">🙏 School of Prayer</Link>
            <Link to="/blog" className="flex items-center gap-2 px-5 py-3.5 bg-white/5 border border-white/10 rounded-2xl no-underline text-gray-300 font-semibold hover:border-white/20 hover:-translate-y-0.5 active:scale-[0.98] transition shadow-sm">✍️ Blog</Link>
          </div>
        </div>
      </section>
