            ...
            {episodes.map((ep, i) => (
              <div key={i} onClick={() => ep.audioUrl && player.openPreview(ep)}
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  <button onClick={e => { e.stopPropagation(); shareUrl(`https://rcm-podcast-react.vercel.app${rssUrl === '' ? '/school-of-prayer' : '/bible-study'}`, `${ep.title} — ${podcastName}`) }}
                    className="w-9 h-9 rounded-full bg-gray-100 hover:bg-gray-200 flex items-center justify-center transition text-gray-500" title="Share">
